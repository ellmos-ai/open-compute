...
_TOOLS: dict[str, dict[str, str]] = {
    ...
    "do": {
        "en": "Execute one canonical action, or a batch, on the desktop (click/type/key/scroll/drag/move). Coordinates are normalized 0..1; state-changing actions pass the safety gate.",
        "de": "Führt eine kanonische Aktion oder einen Stapel auf dem Desktop aus (Klick/Tippen/Taste/Scrollen/Ziehen/Bewegen). Koordinaten sind normiert 0..1; zustandsverändernde Aktionen passieren das Safety-Gate.",
        "es": "Ejecuta una acción canónica, o un lote, en el escritorio (clic/escribir/tecla/desplazar/arrastrar/mover). Las coordenadas están normalizadas 0..1; las acciones que cambian el estado pasan por la barrera de seguridad.",
        "ja": "デスクトップ上で正規アクション（クリック／入力／キー／スクロール／ドラッグ／移動）を1つ、またはバッチで実行します。座標は0..1に正規化され、状態を変更するアクションは安全ゲートを通過します。",
        "ru": "Выполняет одно каноническое действие или пакет на рабочем столе (клик/ввод/клавиша/прокрутка/перетаскивание/перемещение). Координаты нормализованы 0..1; действия, изменяющие состояние, проходят через защитный шлюз.",
        "zh": "在桌面上执行一个规范动作或一批动作（点击/输入/按键/滚动/拖动/移动）。坐标归一化为 0..1；改变状态的动作需通过安全门控。",
    },
    "tree": {
