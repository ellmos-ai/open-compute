...
_TOOLS: dict[str, dict[str, str]] = {
    ...
    "capture_filtered": {
        "en": "Capture only the visual lens allowed by a host-supplied filter profile and blank excluded windows before returning the PNG. Accepts `mode` like `observe_filtered`; in the watching modes an image is an escalation and `reason` is required.",
        "de": "Erfasst nur die vom Host-Filterprofil erlaubte visuelle Linse und blendet ausgeschlossene Fenster vor der PNG-Rückgabe aus. Nimmt `mode` wie `observe_filtered`; in den Beobachtungsmodi ist ein Bild eine Eskalation und `reason` ist Pflicht.",
        "es": "Captura solo la lente visual permitida por un perfil de filtro del host y oculta las ventanas excluidas antes de devolver el PNG. Acepta `mode` como `observe_filtered`; en los modos de observación una imagen es una escalada y `reason` es obligatorio.",
        "ja": "ホスト指定のフィルタープロファイルで許可された視覚レンズのみを撮影し、除外ウィンドウを塗りつぶしてから PNG を返します。 `observe_filtered` と同様に `mode` を受け取ります。観察モードでは画像はエスカレーションであり、`reason` が必須です。",
        "ru": "Захватывает только визуальную область, разрешённую профилем фильтра хоста, и скрывает исключённые окна перед возвратом PNG. Принимает `mode`, как `observe_filtered`; в режимах наблюдения изображение — это эскалация, и `reason` обязателен.",
        "zh": "仅截取主机提供的过滤配置所允许的视觉区域，并在返回 PNG 前遮蔽排除的窗口。 与 `observe_filtered` 一样接受 `mode`；在观察模式下图像属于升级操作，必须提供 `reason`。",
    },
    "do": {
    ...
    "observe_filtered": {
        "en": "Filter the local UIA tree around the declared focus using a strict host profile; return only bounded relevant semantics and metrics. Instead of a full profile, pass `mode`: `observe-lite` (frugal, text only), `observe-full` or `act`.",
        "de": "Filtert den lokalen UIA-Baum anhand eines strikten Hostprofils um den erklärten Fokus; gibt nur begrenzte relevante Semantik und Metriken zurück. Statt eines vollen Profils genügt `mode`: `observe-lite` (sparsam, nur Text), `observe-full` oder `act`.",
        "es": "Filtra el árbol UIA local alrededor del foco declarado mediante un perfil estricto del host; devuelve solo semántica relevante acotada y métricas. En lugar de un perfil completo, pasa `mode`: `observe-lite` (frugal, solo texto), `observe-full` o `act`.",
        "ja": "厳格なホストプロファイルで指定フォーカス周辺のローカル UIA ツリーを絞り込み、制限された関連セマンティクスと指標のみを返します。 完全なプロファイルの代わりに `mode` を渡せます: `observe-lite`（節約・テキストのみ）、`observe-full`、`act`。",
        "ru": "Фильтрует локальное дерево UIA вокруг заданного фокуса по строгому профилю хоста; возвращает только ограниченную релевантную семантику и метрики. Вместо полного профиля можно передать `mode`: `observe-lite` (экономный, только текст), `observe-full` или `act`.",
        "zh": "使用严格的主机配置过滤声明焦点周围的本地 UIA 树，仅返回有界的相关语义和指标。 也可以用 `mode` 代替完整配置：`observe-lite`（节省，仅文本）、`observe-full` 或 `act`。",
    },
    "click_name": {
